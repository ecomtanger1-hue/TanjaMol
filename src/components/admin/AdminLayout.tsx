        ...
        {adminMobileNav.map(item => {
          ...
          return (
            <button
              ...
              onClick={() => onNavigate(item.route)}
              aria-current={active ? 'page' : undefined}
              className={`tm-admin-press grid min-h-[54px] content-center justify-items-center gap-1 rounded-md px-1 text-[11px] font-black leading-tight transition-colors ${active ? 'bg-[#131921] text-white shadow-[0_12px_24px_-18px_rgba(19,25,33,0.8)]' : 'bg-white text-[#65716a] shadow-[inset_0_0_0_1px_rgba(23,32,27,0.08)]'}`}
            >
