  ...
  return <article className="tm-lift tm-surface overflow-hidden rounded-lg bg-[#fffdf8]">
    <button type="button" onClick={openProduct} className="block w-full text-right">
      ...
      <div className={compact ? 'p-2.5' : 'p-3 sm:p-4'}>
        <p className="text-xs font-extrabold text-[#00a66c]">{product.category}</p>
        ...
        <div className="mt-3 flex items-end justify-between gap-2">
          <div>
            <p className="tm-num font-heading text-lg font-black text-[#0f7d55] sm:text-2xl">{product.priceLabel}</p>
            <p className="tm-num text-[11px] font-bold text-[#9a9f9b] line-through sm:text-sm">{product.oldPrice}</p>
          </div>
          <span className="tm-press min-h-[44px] rounded-md bg-[#00a66c] px-3 py-3 text-xs font-extrabold text-white sm:px-4 sm:text-sm">{'\u0627\u0637\u0644\u0628\u0647'}</span>
        </div>
      </div>
    </button>
  </article>;
  ...
  return <div dir="rtl" className="min-h-screen w-full overflow-x-hidden bg-[var(--tm-bg)] pb-24 pt-16 text-[var(--tm-ink)] md:pb-0">
    <header className="tm-site-header fixed inset-x-0 top-0 z-50 text-white shadow-[0_1px_0_rgba(255,255,255,0.08)]" style={{ background: 'var(--tm-header-alpha)' }}>
      <nav className="mx-auto grid min-h-[64px] w-full max-w-[1180px] grid-cols-[44px_1fr_auto] items-center gap-3 px-4 sm:px-6 lg:flex lg:justify-between lg:px-8">
        ...
        <button type="button" onClick={() => scrollToSection('top')} className="tm-press mx-auto flex min-w-0 items-center gap-3 text-center lg:mx-0 lg:text-right">
          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-lg font-heading text-lg font-black shadow-[0_10px_28px_rgba(0,166,108,0.32)]" style={{ background: 'var(--tm-brand)' }}>
            T
          </div>
    ...
    <main id="top">
      ...
      <section id="categories" className="scroll-mt-20 bg-[var(--tm-surface-white)]">
        <div className="mx-auto w-full max-w-[1180px]">
          <div className="tm-reveal-stagger flex gap-4 overflow-x-auto px-4 py-4 sm:px-6 lg:justify-center lg:gap-8 lg:overflow-visible lg:px-8 lg:py-6">
            {shortcutCategories.map(category => (
              <button key={category.id} type="button" onClick={() => navigate(categoryRoute(category.id))} className="tm-reveal-item tm-press w-[88px] shrink-0 text-center sm:w-[96px] lg:w-[116px]">
                <span className="relative mx-auto grid h-16 w-16 place-items-center overflow-hidden rounded-full bg-[#eaf4ef] shadow-[0_0_0_1px_rgba(0,0,0,0.06),0_10px_24px_rgba(23,32,27,0.08)] sm:h-20 sm:w-20 lg:h-24 lg:w-24">
      ...
      <section className="bg-[var(--tm-bg)] px-4 py-4 sm:px-6 lg:px-8 lg:py-6">
        <div className="tm-shell-dark mx-auto grid w-full max-w-[1180px] gap-4 overflow-hidden rounded-lg p-4 text-white sm:p-6 lg:grid-cols-[1fr_430px] lg:items-center lg:gap-8 lg:rounded-2xl lg:p-8">
          <div className="hidden min-w-0 lg:block">
            <p className="inline-flex rounded-full bg-white px-3 py-1 text-xs font-black text-[#0f7d55]">الدفع عند الاستلام</p>
          ...
          <div className="tm-mobile-hero-stage lg:hidden" aria-label="عروض متجر TanjaMol">
            <div className="tm-mobile-hero-slide tm-mobile-hero-intro rounded-lg bg-[#102118] p-5 text-center shadow-[inset_0_0_0_1px_rgba(255,255,255,0.1)]">
              <p className="inline-flex rounded-full bg-white px-3 py-1 text-xs font-black text-[#0f7d55]">TanjaMol</p>
              ...
              <div className="mt-5 flex justify-center gap-2" aria-hidden="true">
                <span className="h-2 w-6 rounded-full bg-[#00d084]" />
          ...
          {featuredProduct ? (
            <button type="button" onClick={() => onOpenProduct(featuredProduct.slug)} className="tm-mobile-hero-slide tm-mobile-hero-product tm-press relative block overflow-hidden rounded-lg bg-white/8 text-right shadow-[0_0_0_1px_rgba(255,255,255,0.1),0_18px_44px_rgba(0,0,0,0.22)]">
              ...
              <span className="absolute inset-0 bg-[linear-gradient(180deg,rgba(16,33,24,0.04),rgba(16,33,24,0.68))]" />
              <span className="absolute bottom-4 right-4 left-4">
                ...
                <span className="tm-num mt-2 block font-heading text-2xl font-black text-[#7de7bd]">{featuredProduct.priceLabel}</span>
              </span>
            </button>
          ) : null}
          </div>
      ...
      <section id="products" className="scroll-mt-20 bg-[var(--tm-surface-white)] py-6 sm:py-8 lg:py-10">
        <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-8">
          ...
          <div className="tm-reveal-stagger mt-4 grid grid-cols-2 gap-2.5 sm:gap-4 lg:grid-cols-4 lg:gap-5">
      ...
      <section className="bg-[#eaf4ef] px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        <div className="mx-auto grid w-full max-w-[1180px] gap-5 overflow-hidden rounded-lg bg-[#102118] p-5 text-white sm:p-8 lg:grid-cols-[1fr_380px] lg:items-center">
          <div>
            <p className="text-sm font-extrabold text-[#7de7bd]">الدفع عند الاستلام</p>
      ...
      <section id="new-arrivals" className="mx-auto w-full max-w-[1180px] scroll-mt-20 px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-sm font-extrabold text-[#0f7d55]">وصل حديثا</p>
          </div>
          <button type="button" className="tm-press tm-button-dark hidden px-5 text-sm sm:block">
            عرض الجديد
          </button>
        </div>
        <div className="tm-reveal-stagger -mx-4 mt-4 flex snap-x gap-3 overflow-x-auto px-4 pb-3 lg:mx-0 lg:grid lg:grid-cols-4 lg:gap-5 lg:overflow-visible lg:px-0 lg:pb-0">
      ...
      <section id="policies" className="bg-[#fffdf7] scroll-mt-20 px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        <div className="mx-auto grid w-full max-w-[1180px] gap-4 lg:grid-cols-[260px_1fr] lg:gap-6">
          <div>
            <p className="text-sm font-extrabold text-[#0f7d55]">قبل الطلب</p>
          </div>
    ...
    <div className="sticky bottom-0 z-40 bg-[var(--tm-surface-white)]/96 px-3 pt-2 shadow-[0_-12px_30px_rgba(23,32,27,0.14),0_0_0_1px_rgba(0,0,0,0.06)] md:hidden" style={{
      ...
      <div className="mx-auto grid max-w-[520px] grid-cols-4 gap-2">
        ...
        <button type="button" onClick={openCategories} className={bottomItemClass}>
          <Grid3X3 className="mx-auto mb-1 h-5 w-5 text-[#0f7d55]" aria-hidden="true" strokeWidth={2.4} />
          {'\u0627\u0644\u0623\u0642\u0633\u0627\u0645'}
        </button>
        ...
        <button type="button" onClick={() => scrollToSection('products')} className={bottomItemClass}>
          <PackageSearch className="mx-auto mb-1 h-5 w-5 text-[#0f7d55]" aria-hidden="true" strokeWidth={2.4} />
          {'\u0627\u0637\u0644\u0628'}
        </button>
      </div>
    </div>
