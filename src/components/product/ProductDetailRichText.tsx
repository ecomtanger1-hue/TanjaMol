...
const titleSizeClass = {
  h2: 'text-[1.8rem] leading-[1.18] sm:text-3xl lg:text-4xl',
  h3: 'text-[1.65rem] leading-[1.2] sm:text-2xl lg:text-3xl',
} as const;
...
export function ProductDetailRichText({ detail }: { detail: ProductDetailBlock }) {
  ...
  if (detail.richTextHtml?.trim()) {
    return (
      <div
        className={`tm-product-rich-text tm-copy tm-text-muted mt-2 grid min-w-0 max-w-full gap-2 overflow-hidden whitespace-normal break-words font-medium leading-7 [overflow-wrap:anywhere] [text-wrap:wrap] lg:mt-3 lg:gap-3 lg:leading-8 ${copySizeClass[detail.textSize || 'base']} ${textAlignClass[detail.textAlign || 'right']} [&_a]:font-extrabold [&_a]:text-[#b45309] [&_a]:underline [&_a]:underline-offset-4 [&_b]:font-extrabold [&_blockquote]:rounded-md [&_blockquote]:border [&_blockquote]:border-[#f0d8b4] [&_blockquote]:bg-[#fff7e8] [&_blockquote]:px-4 [&_blockquote]:py-3 [&_blockquote]:font-bold [&_font[size='2']]:text-xs [&_font[size='3']]:text-sm [&_font[size='4']]:text-lg [&_font[size='5']]:text-2xl [&_font[size='6']]:text-3xl [&_h1]:tm-heading [&_h1]:font-heading [&_h1]:text-4xl [&_h1]:font-black [&_h1]:leading-tight [&_h2]:tm-heading [&_h2]:font-heading [&_h2]:text-3xl [&_h2]:font-black [&_h2]:leading-tight [&_h3]:tm-heading [&_h3]:font-heading [&_h3]:text-2xl [&_h3]:font-black [&_h4]:tm-heading [&_h4]:font-heading [&_h4]:text-xl [&_h4]:font-extrabold [&_hr]:my-4 [&_hr]:border-[#dbe4dd] [&_img]:my-3 [&_img]:max-h-[420px] [&_img]:w-full [&_img]:rounded-md [&_img]:object-contain [&_li]:leading-8 [&_mark]:rounded [&_mark]:px-1 [&_ol]:grid [&_ol]:list-decimal [&_ol]:gap-2 [&_ol]:pr-5 [&_p]:max-w-full [&_p]:break-words [&_p]:[overflow-wrap:anywhere] [&_p]:[text-wrap:wrap] [&_strong]:font-extrabold [&_table]:mt-3 [&_table]:w-full [&_table]:overflow-hidden [&_table]:rounded-md [&_table]:border [&_table]:border-[#dbe4dd] [&_table]:bg-white [&_td]:border [&_td]:border-[#dbe4dd] [&_td]:px-3 [&_td]:py-2 [&_th]:border [&_th]:border-[#dbe4dd] [&_th]:px-3 [&_th]:py-2 [&_ul]:grid [&_ul]:list-disc [&_ul]:gap-2 [&_ul]:pr-5 [&_video]:my-3 [&_video]:max-h-[420px] [&_video]:w-full [&_video]:rounded-md`}
        style={style}
  ...
  return (
    <div
      className={`tm-copy tm-text-muted mt-2 grid gap-2 whitespace-pre-line font-medium leading-7 [text-wrap:wrap] lg:mt-3 lg:gap-3 lg:leading-8 ${copySizeClass[detail.textSize || 'base']} ${textAlignClass[detail.textAlign || 'right']}`}
      style={style}
    >
